...
func get_save_data() -> Dictionary:
	...
	if SaveManager.get_image(_image_time_stamp) == null:
		SaveManager.save_image(_image_time_stamp, _sprite_3d.texture.get_image())
...
func load_save_data(data: Dictionary) -> void:
	if not data.is_empty():
		...
		# Check if the save manager has the image or if the image exists
		var img: Image = SaveManager.get_image(_image_time_stamp)
		if img != null:
			_sprite_3d.texture = ImageTexture.create_from_image(img)
